// FORK DOCUMENTATION:
// This is a workspace fork of ffmpeg-sys-next v6.1.0 (from crates.io, commit unknown as it is a published crate version).
// Reason for fork: to check in pre-generated bindings for Windows MSVC to avoid requiring LLVM/Clang locally.
//
// To regenerate bindings for a target:
//   cargo build -p ffmpeg-sys-next --features generate-bindings
//
// This uses bindgen 0.72.1 (which fixes opaque AVFormatContext structs on MSVC) while preserving
// the original ParseCallbacks (which coerce AV_CODEC_FLAG_* to UInt to avoid E0308 type mismatches).
// The headers used are the standard ones discovered by the build script (e.g., via vcpkg).
//

#[cfg(any(feature = "generate-bindings", not(target_os = "windows")))]
...
fn main() {
    ...
    #[cfg(all(not(feature = "generate-bindings"), target_os = "windows"))]
    {
